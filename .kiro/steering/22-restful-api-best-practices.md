...
|-----------|------|---------|
| Nouns, not verbs | Resources are things, not actions | `/api/orders` not `/api/getOrders` |
...
| Hierarchical relationships | Nest when parent-child is strong | `/api/orders/{id}/lines` |
...
POST   /api/{resource}/{id}/{action} → Non-CRUD action (e.g., /api/orders/{id}/cancel)
```

### Rules

- Use GUIDs for resource IDs in URLs: `/api/orders/{id:guid}`
- Avoid exposing internal/sequential IDs — they leak information about volume
- Actions that don't map to CRUD use `POST` with a verb sub-resource: `/api/orders/{id}/place`
- Query parameters for filtering/sorting: `/api/orders?status=placed&sort=-createdAt`
...
```
GET /api/orders?after=01234567-89ab-cdef&limit=20
```
...
```
GET /api/orders?status=placed&customerId=abc123
...
```
GET /api/orders?sort=createdAt        → ascending
GET /api/orders?sort=-createdAt       → descending (prefix with -)
GET /api/orders?sort=-createdAt,status → multiple sort fields
```
...
group.MapGet("/{id:guid}", handler)
    .CacheOutput(p => p.Expire(TimeSpan.FromHours(1)).Tag("orders"));
...
| User-specific mutable data | `private, no-cache` | User's orders |

...
export function OrderPage() {
  ...
  useEffect(() => { fetch('/api/orders').then(...).catch(...) }, []);
...
export function OrderPage() {
  const { data, isLoading, error } = useOrders();       // Data fetching → custom hook
  const mutation = usePlaceOrder();                       // Mutations → TanStack Query

  if (isLoading) return <LoadingIndicator />;
  if (error) return <div role="alert">{error.message}</div>;

  return <OrderList orders={data} onPlace={mutation.mutate} />;
}
```
